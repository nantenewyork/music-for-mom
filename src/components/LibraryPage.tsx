        ...
        <div className="impressionist-bg body-sans min-h-screen overflow-x-hidden" style={{ color: colors.warmSlate }}>
            ...
            <main className="flex-1 flex flex-col items-center px-6 py-12 lg:py-20">
                ...
                <div className="mb-12 text-center fade-in">
                    ...
                    >
                        <span className="material-symbols-outlined text-sm" style={{ color: colors.deepGold }}>favorite</span>
                        <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: colors.deepGold }}>My Collection</span>
                    </div>
                    ...
                    <p className="mt-4 text-lg" style={{ color: `${colors.warmSlate}99` }}>
                        {savedMusic.length > 0 
                ...
                {savedMusic.length === 0 && (
                    <div className="w-full max-w-2xl fade-in">
                        <div 
                            className="glass-panel-warm painterly-shadow rounded-[2rem] p-12 text-center"
                        >
                            <div 
                                className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full"
                                style={{ backgroundColor: `${colors.deepGold}1a` }}
                            >
                                <span className="material-symbols-outlined text-5xl" style={{ color: colors.deepGold }}>library_music</span>
                            </div>
                            <h3 className="premium-serif text-2xl font-medium mb-3" style={{ color: colors.warmSlate }}>
                                라이브러리가 비어있어요
                            </h3>
                            <p className="text-base mb-8" style={{ color: `${colors.warmSlate}99` }}>
                                음악 추천을 받고 마음에 드는 곡을 저장해보세요.<br />
                                당신만의 클래식 컬렉션을 만들 수 있어요.
                            </p>
                            <button
                                onClick={onBack}
                                className="yt-button inline-flex items-center justify-center gap-2 rounded-full px-8 py-4 text-base font-bold text-white shadow-xl"
                            >
                    ...
                    <div className="w-full max-w-6xl fade-in">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {savedMusic.map((music, index) => (
                                <div 
                                    key={music.id}
                                    className="glass-panel-warm painterly-shadow rounded-[1.5rem] overflow-hidden group hover:scale-[1.02] transition-all duration-300"
                                >
                                    {/* Thumbnail */}
                                    <div 
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                                            <button
                                                ...
                                                className="h-16 w-16 rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center border border-white/40 hover:scale-110 transition-transform"
                                            >
                                                <span className="material-symbols-outlined text-3xl" style={{ color: colors.deepGold }}>play_arrow</span>
                                            </button>
                                        </div>
                                        {/* Mood Badge */}
                                        <div className="absolute top-4 left-4">
                                            <span 
                                                className="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold backdrop-blur-md"
                                        ...
                                        <button
                                            ...
                                            className="absolute top-4 right-4 h-8 w-8 rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-red-100"
                                        >
                                    ...
                                    <div className="p-6">
                                        <h3 
                                            className="premium-serif text-xl font-medium leading-tight mb-1 line-clamp-2"
                                            style={{ color: colors.warmSlate }}
                                        >
                                            {music.title}
                                        </h3>
                                        <p 
                                            className="text-base italic premium-serif mb-3"
                        ...
                        <div className="mt-12 text-center">
                            <button
                                onClick={onBack}
                                className="inline-flex items-center justify-center gap-2 rounded-full bg-white/40 px-8 py-4 text-base font-bold hover:bg-white/60 transition-all shadow-sm"
